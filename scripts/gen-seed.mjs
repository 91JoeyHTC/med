...
for (const id of [5,6,7,8]) { const e = sat[String(id)].elem; rows.push(`(${id},'family',${esc(spec.positionNames[String(id)])},null,null,null,${esc(e)},${esc(spec.elements[e].family)},null,null,null,null,null,${kb(id)})`); }
// 9-12 direction
for (const dir of ['北','東','南','西']) { const d = spec.directions[dir]; rows.push(`(${d.id},'direction',${esc(spec.positionNames[String(d.id)])},null,null,null,${esc(d.elem)},${esc(spec.familyOfElement[d.elem])},${esc(dir)},null,null,null,null,${kb(d.id)})`); }
// 13-24 moons
